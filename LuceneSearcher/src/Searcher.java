...
public class Searcher {
	...
	//the original.
	private static String findMoreRelevantWord(String word) throws IOException, ParseException {
		String[] suggestions = suggestWords(word, 5, 0.6f);
		if (suggestions.length != 0) {
			for (int i = 0; i < 2 && i < suggestions.length; i++) {
				String newWord = suggestions[i];
				TopDocs resOriginalWord = searcher.search(parser.parse(word), 10);
	...
	private static String[] suggestWords(String word, int numberOfSuggestions, float accuracy) {
		try {
	...
	private static void resultsListerAndNavigator(BufferedReader in, ScoreDoc[] hits, int hitsPerPage) throws IOException {
