...
public class Searcher {
	...
	public ResultsOfSearch search(boolean forced) throws Exception {
		...
		try {	
				...
				TopDocs results = luceneSearcher.search(query, 5 * hitsPerPage);
				ScoreDoc[] hits = results.scoreDocs;
				if (!forced)
					if (results.scoreDocs.length == 0 || results.getMaxScore() <= SCORE_THRESHOLD) {
						Query newQuery = suggestQuery(query, FIELD);
						TopDocs newResults = luceneSearcher.search(newQuery, 5 * hitsPerPage);
	...
	private DocumentResult[][] createDocumentsResulted(ScoreDoc[] hits, Query query) throws IOException, InvalidTokenOffsetsException {
		...
		DocumentResult[][] docResArray = new DocumentResult[rows][cols];		
